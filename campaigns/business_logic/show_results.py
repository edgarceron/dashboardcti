...
def headers_csv(quesitons):
    ...
    for header in quesitons:
        row[header.id] = header.text.encode("utf-8").decode("utf-8")
    return row
...
def collect_data(id_campaign, start_date="", end_date=""):
    ...
    for header in headers:
        ...
        row['telefono'] = data_llamada.telefono.encode("utf-8").decode("utf-8")
        row['name'] = data_llamada.name.encode("utf-8").decode("utf-8")
        row['cedula'] = data_llamada.cedula.encode("utf-8").decode("utf-8")
        row['correo'] = data_llamada.correo.encode("utf-8").decode("utf-8")
        row['placa'] = data_llamada.placa.encode("utf-8").decode("utf-8")
        row['linea_veh'] = data_llamada.linea_veh.encode("utf-8").decode("utf-8")
